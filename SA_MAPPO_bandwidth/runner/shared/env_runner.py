...
class EnvRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            if self.use_linear_lr_decay: #启用线性学习率衰减策略，每隔一定episode数降低学习率
            ...
            if episode % self.log_interval == 0:
                ...
                print("average episoderewards  is {}".format(train_infos["average_episode_rewards"]))
    ...
    def warmup(self):
        ...
        # replay buffer 
        if self.use_centralized_V: #CTDE
            # 当使用集中式价值函数时
            # 原始观测形状 [env_num, agent_num, obs_dim]
            share_obs = obs.reshape(self.n_rollout_threads, -1)  # 步骤1：展平智能体维度
            # 新形状 [env_num, agent_num*obs_dim]
            
            share_obs = np.expand_dims(share_obs, 1).repeat(
                self.num_agents, axis=1
            )  # 步骤2：扩展维度并复制
            # 最终形状 [env_num, agent_num, agent_num*obs_dim]
        else:
            # 不使用集中式价值函数时
            share_obs = obs  # 保持原始形状 [env_num, agent_num, obs_dim]
