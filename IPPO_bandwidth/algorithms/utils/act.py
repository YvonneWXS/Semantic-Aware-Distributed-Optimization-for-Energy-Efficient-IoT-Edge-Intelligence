...
class ACTLayer(nn.Module):
    ...
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain):
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            for action_dim in action_dims:
                self.action_outs.append(Categorical(inputs_dim, action_dim, use_orthogonal, gain)) #inputs_dim是hidden_size
    ...
    def forward(self, x, available_actions=None, deterministic=False):
        ...
        if self.mixed_action:
            ...
            for action_out in self.action_outs:
                action_logit = action_out(x)
                action = action_logit.mode() if deterministic else action_logit.sample()  #确定性模式：选择均值动作部署；随机采样模式：按概率分布采样，用于训练
                action_log_prob = action_logit.log_probs(action)
