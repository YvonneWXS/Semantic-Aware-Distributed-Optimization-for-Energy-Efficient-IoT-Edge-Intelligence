...
class DiscreteActionEnv(object):
    ...
    def __init__(self):
        ...
        share_obs_dim = 0
        # 为每个维度定义 [min, max] 范围
        # 动作空间四维：[卸载决策, 语义因子, MEC资源, 带宽权重]
        # 卸载决策: 0或1 (2个值)
        # 语义因子: 离散化为8个值，索引0-7，对应0.3-1.0
        # MEC资源: 离散化为10个值，索引0-9，对应0.1-1.0 (归一化比例)
        # 带宽权重: 离散化为4个值，0-3
        action_space_params = [
            [0, 1],  # offload_decision: 0 or 1
            [0, 7],   # semantic_factor: 8 discrete values (0.3-1.0)
            [0, 9],   # resource_allocation: 10 discrete values (0.1-1.0)
            [0, 3]    # bandwidth_weight: 0,1,2,3
        ] #🌟
        self.action_space = MultiDiscrete(action_space_params)  
        # total_action_space = []
        # for offload in offload_decision:
        #     if offload == 0:
        #         total_action_space.append([offload, 1.0, 0.0])
        #     else:
        #         for semantic in semantic_factor:
        #             if semantic > semantic_threshold:
        #                 for resource in resource_allocation:
        #                     total_action_space.append([offload, semantic, resource])
        ...
        for agent_idx in range(self.num_agent):
            # action space
            # self.action_space.append(action_space[agent_idx])
            # observation space
...
class MultiDiscrete:
    ...
    def __init__(self, array_of_param_array):
        ...
        self.high = np.array([x[1] if len(x) > 1 else 0 for x in array_of_param_array])
