        ...
        for agent_idx in range(self.num_agents):
            ...
            with torch.no_grad():
                ...
                for i in range(self.num_agents):
                    ...
                    # 创建一个独立的随机生成器，每次生成新的噪声
                    # rng = torch.Generator()
                    # rng.manual_seed(torch.randint(0, 1_000_000, (1,)).item())  # 每次随机种子不同
                    # noise: torch.Tensor = torch.randn(next_action_i.shape, generator=rng, device=next_action_i.device) * config.TARGET_POLICY_NOISE
                ...
                agent_reward = agent_reward* 0.000001  # TODO reward缩放防止过大导致的爆炸
                # agent_reward = torch.clamp(agent_reward, -1e3, 1e3) 
