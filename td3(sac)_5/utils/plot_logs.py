...
def generate_plots(log_file: str, output_dir: str, output_file_prefix: str, timestamp: str) -> None:
    ...
    if "update" in log_data[0]:
        ...
        return
    parameters: dict = {x_axis_key: [entry[x_axis_key] for entry in log_data], "reward": [entry["reward"] for entry in log_data], "energy": [entry["energy"] for entry in log_data], "violation_rate": [entry["violation_rate"] for entry in log_data]}
    metrics: list[str] = ["reward", "energy", "violation_rate"]
