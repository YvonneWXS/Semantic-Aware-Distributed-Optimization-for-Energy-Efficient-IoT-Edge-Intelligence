...
class Log:
    def __init__(self) -> None:
        ...
        self.violation_rates: list[float] = []

    def append(self, reward: float, energy: float, violation_rate: float) -> None:
...
class Logger:
    ...
    def log_metrics(self, progress_step: int, log: Log, log_freq: int, elapsed_time: float, name: str) -> None:
        ...
        violation_rates_slice: np.ndarray = np.array(log.violation_rates[-log_freq:])

        reward_avg: float = float(np.mean(rewards_slice))
        energy_avg: float = float(np.mean(energies_slice))
        violation_rate_avg: float = float(np.mean(violation_rates_slice))
        log_msg: str = f"🔄 {name.title()} {progress_step} | " f"Total Reward: {reward_avg:.3f} | " f"Total Energy: {energy_avg:.3f} | " f"Final Violation Rate: {violation_rate_avg:.3f} | " f"Elapsed Time: {elapsed_time:.2f}s\n"

        with open(self.log_file_path, "a", encoding="utf-8") as f:
            f.write(log_msg)

        data_entry: dict = {name.lower(): progress_step, "reward": reward_avg, "energy": energy_avg, "violation_rate": violation_rate_avg, "time": elapsed_time}
        json_data: list[dict] = []
