...
def train_on_policy(env: Env, model: MARLModel, logger: Logger, num_episodes: int) -> None:
    ...
    buffer: RolloutBuffer = RolloutBuffer(num_agents=config.NUM_UES, obs_dim=config.OBS_DIM_SINGLE, action_dim=config.ACTION_DIM, state_dim=config.STATE_DIM, buffer_size=config.PPO_ROLLOUT_LENGTH, device=model.device)
    ...
    for update in range(1, num_updates + 1):
        ...
        with torch.no_grad():
            ...
            last_values_arr: np.ndarray = np.array([last_value] * config.NUM_UES)
...
def train_off_policy(env: Env, model: MARLModel, logger: Logger, num_episodes: int, total_step_count: int) -> None: 
    ...
    for episode in range(1, num_episodes + 1):
        ...
        episode_violation_rate: float = 0.0
        ...
        for step in range(config.STEPS_PER_EPISODE):
            ...
            if total_step_count <= config.INITIAL_RANDOM_STEPS:
                actions: np.ndarray = np.array([np.random.uniform(-1, 1, config.ACTION_DIM) for _ in range(config.NUM_UES)])
                # actions = np.random.default_rng().uniform(-1, 1, (config.NUM_UES, config.ACTION_DIM)) # 使用numpy的随机数生成器
            else:
                actions = model.select_actions(obs, exploration=True)

            # next_obs, rewards, (total_latency, total_energy, jfi) = env.step(actions, step)
            next_obs, rewards, step_energy, step_violation = env.step(actions, step)
            ...
            if step_violation < 0:
                episode_violation_rate += 1
            if done:
                break

        episode_log.append(episode_reward/config.STEPS_PER_EPISODE, episode_energy/config.STEPS_PER_EPISODE, episode_violation_rate/config.STEPS_PER_EPISODE)
