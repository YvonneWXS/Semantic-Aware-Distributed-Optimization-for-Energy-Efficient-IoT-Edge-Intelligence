...
bw_weight_set = np.array([0, 1, 2, 3])  # 离散带宽权重
...
def initialize_population(pop_size, K ,env, observation, max_attempts=100):
    ...
    while len(population) < pop_size:
        for _ in range(max_attempts):
            ...
            # 生成带宽权重，保证一致性：offload=0 -> bw_weight=0; offload=1 -> bw_weight in {1,2,3}
            bw_weight = np.zeros(K, dtype=int)
            for i in range(K):
                if offload_decision[i] == 1:
                    bw_weight[i] = np.random.choice([1, 2, 3])
                else:
                    bw_weight[i] = 0
            ...
            total_energy, delay_penalty = env.compute_energy_and_delay(offload_decision, semantic_factor, resource_allocation, transmission_power, bw_weight, observation)
            if delay_penalty == 0:
                population.append((offload_decision, semantic_factor, resource_allocation, transmission_power, bw_weight))
                break
    return population

def fitness(offload_decision, semantic_factor, resource_allocation, transmission_power, bw_weight, env, observation, generation, max_generations):
    total_energy, delay_penalty = env.compute_energy_and_delay(offload_decision, semantic_factor, resource_allocation, transmission_power, bw_weight, observation)
...
def mutation(individual, env, observation, mutation_rate=0.9, min_resource=1e-6, max_attempts=50):
    mu, beta, f_mec, p_tx, bw = individual
    for _ in range(max_attempts):
        # 变异offload_decision
        if np.random.rand() < mutation_rate:
            idx = np.random.randint(len(mu))
            mu[idx] = 1 - mu[idx]
            # 如果offload变为0，带宽权重也设为0；如果变为1，设为{1,2,3}中随机值
            if mu[idx] == 0:
                bw[idx] = 0
            else:
                bw[idx] = np.random.choice([1, 2, 3])
        ...
        # 变异带宽权重（只对offload=1的个体）
        if np.random.rand() < mutation_rate:
            idx = np.random.randint(len(bw))
            if mu[idx] == 1:  # 只有卸载用户才变异带宽权重
                current_val = bw[idx]
                candidates = [w for w in [1, 2, 3] if w != current_val]
                if candidates:
                    bw[idx] = np.random.choice(candidates)
        ...
        # 计算并检查约束
        _, delay_penalty = env.compute_energy_and_delay(mu, beta, f_mec, p_tx, bw, observation)
        if delay_penalty == 0:
            return mu, beta, f_mec, p_tx, bw
