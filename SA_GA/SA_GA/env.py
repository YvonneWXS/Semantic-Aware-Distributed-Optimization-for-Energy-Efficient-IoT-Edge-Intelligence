...
class ENV():
    def __init__(self, UEs, MECs, k):
        ...
        # 带宽分配参数
        self.delta_b = 1 * self.kHz  # 资源块大小（RB）
        self.epsilon = 1e-10  # 防止除零的小常数

    def allocate_bandwidth(self, bw_weights):
        """根据离散权重分配带宽
        Args:
            bw_weights: array of shape (UEs,), 每个UE的带宽权重 (0-3)
        Returns:
            allocated_bandwidths: array of shape (UEs,), 每个UE分配到的带宽 (Hz)
        """
        ...
        # 确保非负
        allocated_bandwidth = np.maximum(allocated_bandwidth, 0.0)
    ...
    def compute_energy_and_delay(self, offload_decision, semantic_factor, resource_allocation, transmission_power, bw_weights, observation):
        UE_energy=0
        delay_total_penalty = 0
        offload_num = np.sum(offload_decision)
        # 根据离散权重分配带宽
        allocated_bandwidths = self.allocate_bandwidth(bw_weights)
        # 注：allocated_bandwidths[i] 可能为0，即使 offload_decision[i]==1，但遗传算法应保证一致性            
        for i in range(self.UEs):
            ...
            if offload_decision[i] == 0:# 本地处理
                ...
                B_i = allocated_bandwidths[i]
                if B_i <= 0:
                    # 如果分配带宽为0，视为无效，给予极大惩罚（但遗传算法应保证一致性）
                    uplink_rate = 1e-10  # 极小值避免除零
                else:
                    uplink_rate = B_i * math.log2(1 + transmission_power[i] * channel_gain / (B_i * self.noise_power))
