...
def initialize_population(pop_size, K ,env, observation, max_attempts=100):
    ...
    while len(population) < pop_size:
        for _ in range(max_attempts):
            offload_decision = np.random.choice([0, 1], size=K)  # 随机二元卸载决策
            semantic_factor = np.random.uniform(0.5, 1, size=K)  # 语义提取因子（可调整下限）
            resource_allocation = np.random.uniform(1e-6, 1, size=K)  # 计算资源分配
            resource_allocation = np.maximum(resource_allocation, 1e-6)
            ...
            total_energy, delay_penalty = env.compute_energy_and_delay(offload_decision, semantic_factor,resource_allocation, observation)
            if delay_penalty == 0:
                population.append((offload_decision, semantic_factor, resource_allocation))
                break
    return population

# def fitness(offload_decision, semantic_factor, resource_allocation, env, observation, generation, max_generations):
#     total_energy, delay_penalty = env.compute_energy_and_delay(offload_decision, semantic_factor, resource_allocation, observation)
    
#     penalty_weight = 10 + (generation / max_generations)  # 自适应惩罚
#     penalty = penalty_weight * delay_penalty
#     total_energy += penalty  # 加上惩罚项
    
#     return -total_energy  # 适应度为负，因我们要最小化能耗
...
def crossover(parent1, parent2, crossover_rate=0.89):
    ...
    if np.random.rand() < crossover_rate:
        ...
        child1 = (np.concatenate((parent1[0][:point], parent2[0][point:])),
                  np.concatenate((parent1[1][:point], parent2[1][point:])),
                  np.concatenate((parent1[2][:point], parent2[2][point:])))
        child2 = (np.concatenate((parent2[0][:point], parent1[0][point:])),
...
def mutation(individual, env, observation, mutation_rate=0.9, min_resource=1e-6, max_attempts=50):
    for _ in range(max_attempts):
        """随机变异"""
        mu, beta, f_mec = individual
    
        if np.random.rand() < mutation_rate:
            idx = np.random.randint(len(mu))
            mu[idx] = 1 - mu[idx]  # 变异卸载决策
        
        if np.random.rand() < mutation_rate:
            idx = np.random.randint(len(beta))
            beta[idx] = np.clip(beta[idx] + np.random.uniform(-0.05, 0.05), 0.5, 1)
        
        if np.random.rand() < mutation_rate:
            idx = np.random.randint(len(f_mec))
            f_mec[idx] = np.clip(f_mec[idx] + np.random.uniform(-0.05, 0.05), min_resource, 1)
        
        f_mec = np.maximum(f_mec, min_resource)
        if np.sum(f_mec) > 0:
            f_mec /= np.sum(f_mec)

        # 检查 delay_penalty 是否为 0
        _, delay_penalty = env.compute_energy_and_delay(mu, beta, f_mec, observation)
        if delay_penalty == 0:
            return mu, beta, f_mec
...
if __name__ == '__main__':
    ...
    # 保存为 CSV 文件
    df.to_csv("GA_energy_vs_UEs.csv", index=False)
    print("Results saved to GA_energy_vs_UEs.csv")
    print(df)
